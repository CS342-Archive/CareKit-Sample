...
class CKSendHelper {
    ...
    */
    static func sendToFirestore(data: Data, collection: String, withIdentifier identifier: String? = nil, onCompletion: ((Bool, Error?) -> Void)? = nil) throws {
        ...
        return CKSendHelper.sendToFirestore(json: dictionary!, collection: collection, withIdentifier: identifier, onCompletion: onCompletion)
    }
    ...
    */
    static func sendToCloudStorage(_ files: URL, collection: String, withIdentifier identifier: String? = nil) throws {
        ...
        for file in fileURLs {
            ...
            let ref = storageRef.child("\(authCollection)storage/\(collection)/\(identifier ?? UUID().uuidString)/\(file.lastPathComponent)")
